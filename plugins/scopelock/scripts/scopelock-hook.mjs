...
async function sessionStart(input, projectRoot, context) {
  ...
  let health = "unknown";
  let statusNote = "Run $scope-status for a current comparison.";
  try {
    ...
    else if (health === "attention") statusNote = "The current comparison needs attention. Run $scope-status before continuing.";
    else statusNote = `The current comparison is ${sanitizeText(health, 80)}. Run $scope-status before relying on it.`;
  } catch {
    statusNote = "The advisory startup comparison was unavailable. Run $scope-status before relying on the Lock.";
  }
  const text = [
    ...
    "ScopeLock is advisory only. It does not block every write. Use $scope-verify before closing the task.",
  ].filter(Boolean).join(" ");
...
async function postToolUse(input, projectRoot, context) {
  ...
  try {
    ...
    return hookOutput(
      "PostToolUse",
      "ScopeLock could not complete its bounded advisory comparison after the tool call. Run $scope-status before relying on the task boundary.",
  ...
  const facts = [
    ...
    "This advisory ran after the tool completed and did not undo or halt it. Run $scope-status for the full report.",
  ].filter(Boolean).join(" ");
...
async function stopHook(input, projectRoot, context) {
  ...
  return {
    systemMessage: outcome
      ...
      : "ScopeLock remains active without a verification report. Run $scope-verify before closing the task.",
  };
}
...
async function dispatch(input) {
  ...
  if (context.result === "unavailable") {
    if (eventName === "SessionStart" || eventName === "PostToolUse") {
      return hookOutput(
        eventName,
        "ScopeLock local state is unavailable. Run $scope-status to inspect it. This hook did not block or undo any tool action.",
