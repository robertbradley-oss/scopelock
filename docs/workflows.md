...
The plugin exposes one `$scopelock` router skill. It selects the Lock lifecycle, Status, or Verify workflow from the user's request while keeping each workflow's safety and output rules separate.
