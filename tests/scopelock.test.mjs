...
import { access, mkdtemp, mkdir, open, readFile, readdir, realpath, rm, symlink, writeFile } from "node:fs/promises";
...
test("plugin manifest and one router skill have the required structure", async () => {
  ...
  const surfaced = [];
  for (const entry of await readdir(path.join(PLUGIN_ROOT, "skills"), { withFileTypes: true })) {
    if (!entry.isDirectory()) continue;
    try {
      await access(path.join(PLUGIN_ROOT, "skills", entry.name, "SKILL.md"));
      surfaced.push(entry.name);
    } catch {}
  }
  assert.deepEqual(surfaced, ["scopelock"]);
  const skillRoot = path.join(PLUGIN_ROOT, "skills", "scopelock");
  const source = await readFile(path.join(skillRoot, "SKILL.md"), "utf8");
  const frontmatter = source.match(/^---\r?\n([\s\S]*?)\r?\n---/);
  assert.ok(frontmatter, "scopelock frontmatter missing");
  assert.deepEqual(frontmatter[1].split(/\r?\n/).map((line) => line.split(":", 1)[0]), ["name", "description"]);
  assert.match(frontmatter[1], /^name: scopelock$/m);
  ...
  assert.match(agent, /default_prompt:.*\$scopelock/);
